...
- **`DockedWindow`**: Abstract base for a window that docks into the dockspace `DrawDeferredDocked()` creates — subclass it, override `Title` and `DrawContent()`, then call `Show()`/`Close()`
...
**Deferred drawing**: The dialogs above and `DockedWindow` only draw when a per-frame pump runs. Call `ImGuiWidgets.DrawDeferred()` once per frame (at the end of `OnRender`) to draw every open dialog, message box and popup and advance Hexa's animation clock; call `ImGuiWidgets.DrawDeferredDocked()` instead if you use `DockedWindow` — it additionally creates a dockspace over the main viewport and already does everything `DrawDeferred()` does, so call only one of the two per frame (calling both draws every dialog twice). Showing a dialog before either pump has ever run throws `InvalidOperationException`. A pump is not needed just to keep animated widgets like `ToggleSwitch` correct — it self-ticks when unpumped — only to show dialogs or docked windows.
